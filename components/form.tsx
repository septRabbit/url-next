    ...
    <div>
      <div className="relative flex flex-col items-center justify-start">
        ...
        <input
          ...
          name={id}
          className={clsx(
            "w-full p-2 px-3 rounded peer bg-gray-100",
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // const response = await axios("https://miu.services/backend/api/short", {
        const response = await axios("http://35.221.200.152:5000/api/short", {
        method: "POST",
        data: sendData,
      });
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <label htmlFor="Url-live-time">
          URL Live Time:
          <select
            className="flex-1 p-2 ml-4 text-blue-600 border border-blue-500 rounded"
            name="liveTime"
            id="liveTime"
          >
